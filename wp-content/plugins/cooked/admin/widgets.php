...
class cooked_single_recipe_widget extends WP_Widget {
    ...
    public function __construct() {
		$widget_ops = array( 
			'classname' => 'cooked_recipe_widget',
			'description' => 'A widget to display a single recipe.',
		);
		parent::__construct( 'cooked_recipe_widget', esc_html__('Single Recipe','booked'), $widget_ops );
	}
    ...
    function widget($args, $instance) {
		...
		$entry_link = get_permalink($recipe->ID);
		$entry_image = get_post_meta($recipe->ID, '_thumbnail_id', true);
		$entry_title = get_the_title($recipe->ID);
		$entry_rating = cp_recipe_rating($recipe->ID);
		$entry_description = get_post_meta($recipe->ID, '_cp_recipe_short_description', true);
		$entry_excerpt = get_post_meta($recipe->ID, '_cp_recipe_excerpt', true);
		$prep_time = get_post_meta($recipe->ID, '_cp_recipe_prep_time', true);
		$private_recipe = get_post_meta($recipe->ID, '_cp_private_recipe', true);
		$cook_time = get_post_meta($recipe->ID, '_cp_recipe_cook_time', true);
		$total_time = $prep_time + $cook_time;
		$entry_yields = get_post_meta($recipe->ID, '_cp_recipe_yields', true); ?>
						...
						<div class="cp-box-entry">
							<h2><a href="<?php echo $entry_link; ?>"><?php echo $entry_title; ?></a><?php
								if (in_array('difficulty_level', $recipe_info)) :
									$difficulty_level = get_post_meta($recipe->ID, '_cp_recipe_difficulty_level', true);
							...
							<?php if (in_array('description', $recipe_info) && !$hide_desc) : if ($entry_excerpt): echo wpautop($entry_excerpt); else: echo wpautop($entry_description); endif; endif; ?>
							<?php if (in_array('excerpt', $recipe_info) && !in_array('description', $recipe_info) && !$hide_desc && $entry_excerpt) : echo wpautop($entry_excerpt); endif; ?>
...
class cooked_recipe_list_widget extends WP_Widget {
    ...
    public function __construct() {
		$widget_ops = array( 
			'classname' => 'cooked_list_widget',
			'description' => 'A widget to display a recipe list.',
		);
		parent::__construct( 'cooked_list_widget', esc_html__('Recipe List','booked'), $widget_ops );
	}
...
class cooked_recipe_search_widget extends WP_Widget {
	...
	public function __construct() {
		$widget_ops = array( 
			'classname' => 'cooked_search_widget',
			'description' => 'A widget to display the recipe search box.',
		);
		parent::__construct( 'cooked_search_widget', esc_html__('Recipe Search','booked'), $widget_ops );
	}
