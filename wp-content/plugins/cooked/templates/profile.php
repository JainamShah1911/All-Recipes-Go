...
if (empty($user_data)) { $user_data = get_user_by( 'slug', $profile_username ); }

?><div id="cooked-profile-page"<?php if ($my_profile): ?> class="me"<?php endif; ?>><?php

if (is_user_logged_in() && empty($user_data)) {

	echo '<h2>' . __('No profile here!','cooked') . '</h2>';
	echo '<p>' . __('Sorry, this user profile does not exist.','cooked') . '</p>';

} else if (!is_user_logged_in() && empty($user_data)) {
	
	echo do_shortcode('[cooked-login]');

} else {
		
	$cooked_allowed_fields = get_option( 'cp_recipes_fes_user_fields', array( 0 => 'website', 1 => 'bio') );
	if (empty($cooked_allowed_fields)): $cooked_allowed_fields = array(); endif;?>
	
	<?php if (!empty($error)):
		foreach($error as $err){
			echo '<p class="cp-form-error">' . $err . '</p>';
		}
	endif; ?>
	...
	<div class="cp-profile-header cookedClearFix">
		...
		<?php
			...
			if (!in_array('website',$cooked_allowed_fields)):
				$user_url = '';
			endif;
			
			if (!in_array('bio',$cooked_allowed_fields)):
				$user_desc = '';
			endif;
		...
		<div class="cp-info"<?php if (!$avatar): ?> style="padding-left:0;"<?php endif; ?>>
			<div class="cp-user">
				...
				<?php if ($my_profile): ?>
					<a class="cp-logout-button" href="<?php echo wp_logout_url(get_permalink($post->ID)); ?>" title="<?php _e('Logout','cooked'); ?>"><?php _e('Logout','cooked'); ?></a>
				<?php endif; ?>
			</div>
		</div>
	...
	<?php // WooCommerce Options
	$cooked_wc_options = get_option( 'cp_wc_options', array() );
	$wc_account_disabled = (is_array($cooked_wc_options) && in_array('disable_account_tab',$cooked_wc_options) ? true : false);
	?>
	
	<ul class="cp-tabs cookedClearFix">
		<?php if (CP_WOOCOMMERCE_ACTIVE && is_user_logged_in() && $my_profile && !$wc_account_disabled): ?><li><a href="#wcaccount"><i class="fa fa-shopping-cart"></i> <?php _e('My Account','cooked'); ?></a></li><?php endif; ?>
		<?php if ($total_recipes): ?><li><a href="#recipes"><i class="fa fa-cutlery"></i><?php echo number_format($total_recipes); ?> <?php _e('Recipes','cooked'); ?></a></li><?php endif; ?>
	...
	<?php if (CP_WOOCOMMERCE_ACTIVE && is_user_logged_in() && $my_profile && !$wc_account_disabled): ?>
	...
	<div id="profile-reviews" class="cp-tab-content">
		...
		<div id="cooked-plugin-page">
			<div class="cooked-result-section table-layout">
				<div class="table-box">
	...
	<?php if ( is_user_logged_in() && $my_profile ) : ?>
        ...
        <form method="post" enctype="multipart/form-data" id="cooked-page-form" action="<?php the_permalink(); ?>">
            ...
            <div class="cookedClearFix">
	            <p class="form-nickname<?php if (!in_array('website',$cooked_allowed_fields)): ?> cp-col-50<?php endif; ?>">
	            ...
	            <p class="form-email<?php if (!in_array('website',$cooked_allowed_fields)): ?> cp-col-50<?php endif; ?>">
	            ...
	            <?php if (in_array('website',$cooked_allowed_fields)): ?>
		            <p class="form-url">
            ...
            <?php if (in_array('bio',$cooked_allowed_fields)): ?>
