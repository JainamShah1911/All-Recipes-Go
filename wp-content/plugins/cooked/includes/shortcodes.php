...
function cp_timer_shortcode($atts, $content = null) {
	...
	//<button id="timer_button">Open Modal</button>
	return '<a id="timer_button'. $GLOBALS['count_modal'].'" data-time-in-seconds="' . $atts['length'] * 60 . '"><i class="fa fa-clock-o"></i> ' . $content . '</a><div id="digital_timer'.$GLOBALS['count_modal'].'" class="modal_y">

  <!-- Modal content -->
  <div class="modal-content_y">
   <span class="close"></span>
  <iframe width="100%" height="100%" scrolling="no" frameborder="0" src="http://localhost/wordpress/digital_timer/index.php?time='.$atts['length'].'"></iframe>
  </div> 	    
...
function cp_search_shortcode($atts, $content = null) {
	...
	ob_start();
	$browse_page = cp_get_browse_page_id();
...
function cp_browse_recipes($atts) {
	...
	$list_view = get_option('cp_recipe_list_view','recipe_grid');

	ob_start();
	
	global $manual_category, $manual_cuisine, $manual_cooking_method, $manual_sort;
	
	$manual_category = ($atts['category'] ? $atts['category'] : get_option('cp_recipe_list_view_category',false));
	$manual_cuisine = ($atts['cuisine'] ? $atts['cuisine'] : get_option('cp_recipe_list_view_cuisine',false));
	$manual_cooking_method = ($atts['method'] ? $atts['method'] : get_option('cp_recipe_list_view_cooking_method',false));
	$manual_sort = ($atts['sort'] ? $atts['sort'] : get_option('cp_recipe_list_view_sort',false));
...
function cp_single_recipe($atts, $content = null){
	...
	if ($atts['id']):
		global $post_id, $cooked_is_embedded;
		$post_id = $atts['id'];
		if (!$atts['show-reviews']): $hide_reviews = true; endif;
		$cooked_is_embedded = true;
		ob_start();
		...
		?><div class="cp-profile-header cookedClearFix directory-pane">
			...
			<div class="cp-info">
				<div class="cp-user">
					...
					<i class="fa fa-cutlery"></i>&nbsp;&nbsp;<?php echo number_format($total_recipes); ?> <?php _e('Recipes','cooked'); ?>&nbsp;&nbsp;&nbsp;
...
function cp_registration_form($first_name, $last_name, $email){
	...
	?><form action="<?php echo $_SERVER['REQUEST_URI']; ?>" method="post" class="wp-user-form">
		...
		<?php do_action('cooked_before_registration_fields'); ?>
		...
		<?php do_action('cooked_after_registration_fields'); ?>
			...
			<div id="cooked-page-form">
				...
				<div id="profile-login" class="cp-tab-content">
					...
					<div class="cp-form-wrap cookedClearFix">
						...
						<?php if (isset($_GET['loginfailed'])): ?><div class="cp-custom-error not-hidden"><?php _e('Sorry, those login credentials are incorrect.','cooked'); ?></div><?php endif; ?>
				...
				<div id="profile-register" class="cp-tab-content">
					<div class="cp-form-wrap cookedClearFix">
						...
						<?php if ( isset($_POST['submit'] ) ) {
					        ...
					        $first_name 	= apply_filters( 'cooked_registration_first_name', sanitize_user($_POST['first_name']) );
					        $last_name 		= apply_filters( 'cooked_registration_last_name', sanitize_user($_POST['last_name']) );
					        $password 		= apply_filters( 'cooked_registration_password', wp_generate_password() );
					        $email      	= apply_filters( 'cooked_registration_email', sanitize_email($_POST['email']) );
					        
					        if (isset($_POST['captcha_word'])):
					        	$captcha_string	= apply_filters( 'cooked_registration_captcha_word', $_POST['captcha_word'] );
					        	$captcha_code	= apply_filters( 'cooked_registration_captcha_code', $_POST['captcha_code'] );
					        	$captcha_word = strtolower($captcha_string);
				...
				<div id="profile-forgot" class="cp-tab-content">
					<div class="cp-form-wrap cookedClearFix">
						...
						<form method="post" action="<?php echo site_url('wp-login.php?action=lostpassword', 'login_post') ?>" class="wp-user-form">
							<p class="username">
								<label for="user_login" class="hide"><?php _e('Username or Email', 'cooked'); ?></label>
								<input type="text" name="user_login" value="" size="20" id="user_login" tabindex="1001" />
							</p>
								
							<?php do_action('login_form', 'resetpass'); ?>
							<input type="submit" name="user-submit" value="<?php _e('Reset my password', 'cooked'); ?>" class="user-submit" tabindex="1002" />
		...
		if (!empty($required_user_roles)):
			...
			if (in_array($current_user_role,$required_user_roles)){
				...
				if (isset($_GET['success'])):
				    ...
				    //Custom Notification
				    echo "<script>console.log('New Recipe Added');</script>";
				    bp_custom_add_notification($post_id);
					// Form has been submitted, display "thank you" notice.
					echo '<div id="cooked-plugin-page">'.wpautop(__('Thank you for submitting a recipe. You can make your recipe private anytime.','cooked').'<br><br><a class="btn" href="'.get_permalink().'">'.__('Submit another recipe?','cooked').'</a>').'</div>';
					...
					?><div id="cooked-plugin-page"><form id="cooked-submit-recipe-form" action="<?php the_permalink(); ?>" method="post" enctype="multipart/form-data">
						...
						<div class="section-row">
							...
							<div class="section-body section-stats">
								<div class="section-title-box">
									<p class="section-stats-first" data-plural="<?php _e('Sections','cooked'); ?>" data-single="<?php _e('Section','cooked'); ?>">0 <?php _e('Sections','cooked'); ?></p> /
									<p class="section-stats-second" data-plural="<?php _e('Ingredients','cooked'); ?>" data-single="<?php _e('Ingredient','cooked'); ?>">0 <?php _e('Ingredients','cooked'); ?></p>
								...
								<p class="hint-p">
									<?php _e('Enter one ingredient per line. Use a double dash to start new section titles. (ex. --Section Title).','cooked'); ?>
								</p><!-- /.hint-p -->
							</div><!-- /.section-body -->
						</div><!-- /.section-row -->
						...
						<div class="section-row">
							...
							<div class="section-body section-stats">
								<div class="section-title-box">
									<p class="section-stats-first" data-plural="<?php _e('Sections','cooked'); ?>" data-single="<?php _e('Section','cooked'); ?>">0 <?php _e('Sections','cooked'); ?></p> /
									<p class="section-stats-second" data-plural="<?php _e('Steps','cooked'); ?>" data-single="<?php _e('Step','cooked'); ?>">0 <?php _e('Steps','cooked'); ?></p>
								</div><!-- /.section-title-box -->
						...
						<?php if (!in_array('excerpt',$recipe_info)): ?><div class="cp-hidden"><?php endif; ?>
						<div class="section-row">
							<div class="section-head">
						...
						<?php if (!in_array('excerpt',$recipe_info)): ?></div><?php endif; ?>
						
						<div class="section-row">
							<div class="section-head">
								<h2><?php _e('Record Recipe Video', 'cooked'); ?></h2>
							</div><!-- /.section-head -->
							<div class="section-body">
								<div class="field-wrap">
									<input style="display:none;" type="text" class="field" name="_cp_recipe_external_video" value="<?php echo (isset($_POST['_cp_recipe_external_video']) ? $_POST['_cp_recipe_external_video'] : ''); ?>" />
                                <!--Modal-->
                                   <a id="add_button">Record</a>
                                   <div id="modal" class="modal_y">
									<!-- Modal content -->
								  <div class="modal-content_d">
								   <span class="close"></span>
								   
								  
								  <iframe width="100%" height="100%" scrolling="no" frameborder="0" src="http://localhost/wordpress/recordrtc/rec2/index.php?file=<?php 

								  global $wpdb;
								  //$file_name = $wpdb->query("SELECT ID FROM `wp_posts` ORDER BY `wp_posts`.`ID` DESC limit 1");
								  $servername = "localhost:3306";
									...
									$sql = "SELECT ID FROM `wp_posts` ORDER BY `wp_posts`.`ID` DESC limit 1";
						...
						<?php if (!in_array('difficulty_level',$recipe_info)): ?><div class="cp-hidden"><?php endif; ?>
							<div class="section-row cookedClearFix">
								...
								<?php cp_difficulty_dropdown('_cp_recipe_difficulty_level', __('Difficulty Level', 'cooked'), $difficulty); ?>
							</div>
							...
							if ($tax_columns): ?>
								<div class="section-row cookedClearFix">
									...
									<?php if (in_array('method',$enabled_taxonomies)):
										...
										<div class="<?php if ($tax_columns == 3) : ?>section-third<?php elseif ($tax_columns == 2) : ?>section-col<?php endif; ?>"><div class="section-head"><h2><?php _e('Budget', 'cooked'); ?></h2></div><?php cp_taxonomy_dropdown('cp_recipe_cooking_method', __('Choose one or more...', 'cooked'),$method); ?></div>
									<?php endif; ?>
							...
							<div class="section-col">
								...
								<div class="section-body"><?php
									...
									?><div class="slider" data-maxval="<?php echo $max_prep_time*60*60; ?>">
										<div class="amount"><input type="text" maxlength="5" value="00:00" class="slider-timer" disabled /> <i class="fa fa-clock-o"></i>
						...
						<input id="submit" type="submit" class="sbmt-button" value="<?php _e('Submit Recipe','cooked'); ?>" />					
						<input type="hidden" name="action" value="cooked-recipe-post" />
						<input type="hidden" name="cooked_submit_page" value="<?php echo esc_url(get_permalink()); ?>" />
...
function cp_recipe_form_submit_post(){
	...
	if ( 'POST' == $_SERVER['REQUEST_METHOD'] && !empty( $_POST['action'] ) && $_POST['action'] == 'cooked-recipe-post' ){
		if ( !is_user_logged_in() )
			return;
		global $current_user;
		$required_user_roles = get_option('cp_recipes_fes_user_roles');
			...
			if (count($error_array) == 0){
				...
				if (count($error_array) == 0){
					...
					if ($email_content && $email_subject):
						...
						$admin_email = cooked_which_admin_to_send_email();
					...
					wp_reset_postdata();
					$cooked_submit_page = (isset($_POST['cooked_submit_page']) ? $_POST['cooked_submit_page'] : get_permalink());				
					wp_redirect(trailingslashit($cooked_submit_page).'?success');
					exit;
...
function cp_recipe_card($atts, $content = null) {
	...
	if ($entry_id):
		...
		$recipe_post = get_post($entry_id);
		$entry_title = $recipe_post->post_title;
		...
		if ($atts['style'] == 'horizontal'): ?>
			...
			<div id="cooked-plugin-page" class="cooked-recipe-card cookedClearFix">	
				<div class="cooked-result-section full-width-box-layout">
					<div class="cooked-result-box item">
						<div class="cp-box">
							<div class="cp-box-img">
								<?php if(!empty($entry_image)) {
									echo '<a href="'.$entry_link.'">'.wp_get_attachment_image($entry_image, 'cp_960_425').'</a>';
								} else {
									echo '<a href="'.$entry_link.'"><img src="'.CP_PLUGIN_URL.'/css/images/default_960_425.png"></a>';
								}
								?>
							</div><!-- /.cp-box-img -->
							<div class="cp-box-info">
								...
								<?php if (in_array('description', $recipe_info)) : if ($entry_excerpt): echo wpautop($entry_excerpt); else: echo wpautop($entry_description); endif; endif; ?>
		...
		<?php else : ?>
			...
			<div id="cooked-plugin-page" class="cooked-recipe-card cookedClearFix">	
				<div class="cooked-result-section cooked-masonry-layout">
					<div class="cooked-result-box item">
						<div class="cp-box">
							<div class="cp-box-img">
								<?php if(!empty($entry_image)) {
