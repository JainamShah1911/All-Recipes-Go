...
if ( ! defined('ABSPATH')) exit;  // if direct access
	...
	$fb_url = get_fbe_field('fb_event_uri');
	//////////////////////////////////////////////////////
	...
	foreach($layout as $item_id=>$item_info){
		...
		elseif($item_key=='title_link'){
				...
				$html_content.= '<a target="'.$link_target.'" class="element element_'.$item_id.' '.$item_key.'" href="'.$fb_url.'">'.apply_filters('post_grid_filter_grid_item_title',wp_trim_words(get_the_title(), $char_limit,'')).'</a>';
		...
		elseif($item_key=='read_more'){
			...
			if(!empty($item_info['read_more_text'])){
				...
				$read_more_text = apply_filters('post_grid_filter_grid_item_read_more', __('View Facebook Page', post_grid_textdomain));
				
				
				}


				$html_content.= '<a class="element element_'.$item_id.' '.$item_key.'" target="_blank" href="'.$fb_url.'">'.$read_more_text.'</a>';
		...
		elseif($item_key=='thumb_link'){
			...
			if(!empty($thumb_url)){
				$html_content.= '<a href="'.$fb_url.'"><img src="'.$thumb_url.'" /></a>';
				}
		...
		elseif($item_key=='excerpt_read_more'){
			...
			$html_content.= wp_trim_words(strip_shortcodes(get_the_excerpt()), $char_limit,'').' <a target="'.$link_target.'" class="read-more" href="'.$fb_url.'">'.$read_more_text.'</a>';
			$html_content.='</div>';
			}
